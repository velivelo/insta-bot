...
class InstaBot ():
    ...
    def login (self):
        ...
        r = self.s.get(self.urls["default"])
    ...
    def follow (self, user_id):
        ...
        r = self.s.post(self.urls["follow"].format(user_id))
        if r.status_code != 200:
            raise Exception('User id : "{}" is attribuate to no one'.format(user_id))
        return r

    @require_loggin
    def unfollow (self, user_id):
        """Unfollow an user by his id"""
        r = self.s.post(self.urls["unfollow"].format(user_id))
        if r.status_code != 200:
            raise Exception('User id : "{}" is attribuate to no one'.format(user_id))
        return r
    ...
    def like (self, media_id):
        ...
        r = self.s.post(self.urls["like"].format(media_id))
        if r.status_code != 200:
            raise Exception("{} is not a valid media id".format(media_id))
    
    @require_loggin
    def unlike (self, media_id):
        """Unlike a media by its id"""
        r = self.s.post(self.urls["unlike"].format(media_id))
        if r.status_code != 200:
            raise Exception("{} is not a valid media id".format(media_id))
    ...
    def comment (self, media_id, text):
        ...
        if r.status_code != 200:
            raise Exception('Media with id: "{}" does\'nt exist')
        return r.json()["id"]

    @require_loggin
    def removeComment (self, media_id, comment_id):
        """Remove a comment from a media by its id"""
        r = self.s.post(self.urls["remove_comment"].format(media_id, comment_id))
        if r.status_code != 200:
            raise Exception('Media with id: "{}" or/and comment with id: "{}" don\'t exist')

    def getUserDetails (self, username):
        """Return an user details into a dictionary from his username"""
        r = self.s.get(self.urls["user_details"].format(username))
        if r.status_code != 200:
            raise Exception('"{}" is an unassigned username'.format(username))
    ...
    def userIdToUsername (self, user_id):
        ...
        r = self.s.get(self.urls["follow"].format(user_id))
        return re.search(r".com/(.+)/", r.url).group(1)

    def explore (self, tag):
        """Return the most recent posted medias containing the tag in argument"""
        r = self.s.get(self.urls["explore"].format(tag))
        if r.status_code != 200: 
